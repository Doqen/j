...
fetch('../articles.json')
    ...
    .then(articles => {
        let entertainmentArticles = articles.filter(article => article.tag === 'Entertainment');
        createArticles(entertainmentArticles);
    })
    .catch(error => console.error(error));

// Function to create articles
function createArticles(articles) {
    const articlesContainer = document.getElementById('entertainmentarticlesContainer'); // Get reference to the container
